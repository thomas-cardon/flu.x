  ...
  post: async function(payload) {
    ...
    for (const [roleName, role] of Object.entries(payload.roles)) {
      ...
      for (const itemset of role.itemsets)
        if (itemset.blocks.length > 0) await ItemSet.updateOne({ championId: payload.championId, role: roleName, gameMode: payload.gameMode, ...itemset }, { ...q, ...itemset }, { upsert: true }).exec();
    }
