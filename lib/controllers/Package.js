...
router.get('/v1/:championId/:role', async function(req, res, next) {
  ...
  try {
    if (req.params.championId === 0 || !req.params.championId || isNaN(req.params.championId)) throw Error('ChampionId must be a non-zero number');
    const data = await Package.find(q, req.query.itemsets === 'true', req.query.summonerspells === 'true', isNaN(req.query.maxperkpages) ? 2 : parseInt(req.query.maxperkpages)) || { statusCode: 404, error: 'DATA_MISSING', message: 'Missing data' };
...
router.post('/v1/upload', async function(req, res, next) {
  if (!req.body.roles) res.json({ statusCode: 404, error: 'ROLES_NOT_FOUND', message: 'Data is incomplete' });
  
  if (await Package.post(req.body)) res.json({ statusCode: 200, message: 'SAVED', success: true });
