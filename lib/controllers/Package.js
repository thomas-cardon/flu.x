...
module.exports = {
  ...
  getByRole: async (req, h, q = { championId: req.params.championId }) => {
    ...
    return await Package.find(q).select(`${req.params.role} championId providerId gameMode`).limit(1).lean().exec() || { statusCode: 404, error: "PERKS_NOT_FOUND", message: "Couldn't find perks" };
  },
  post: async (req, h) => {
