...
router.get('/alerts/v2', async function (req, res, next) {
  ...
  const data = await Alert.find().or([{ affectedSummonerIds: { $in: req.query.summoner ? req.query.summoner.split(',') : [] } }, { affectedVersions: req.query.v }, { affectedSummonerIds: null }, { affectedVersions: null }]).lean().exec();
  res.send(data);
});
