...
router.post('/v1/upload', async function(req, res, next) {
  if (!req.body.roles) return { statusCode: 404, error: 'ROLES_NOT_FOUND', message: 'Data is incomplete' }
  return Package.post(req.body) ? { statusCode: 200, message: 'SAVED', success: true } : { statusCode: 500, message: 'NOT_SAVED' };
});
