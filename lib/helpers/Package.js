...
module.exports = {
  ...
  postv3: async function(payload) {
    ...
    for (const [roleName, role] of Object.entries(payload.roles)) {
      q = { championId: payload.championId, gameMode: roleName === 'ARAM' ? 'ARAM' : (role.gameMode || payload.gameMode), gameVersion: payload.gameVersion, gameRegion: payload.gameRegion, role: roleName };

      if (role.summonerspells.length >= 2) await SummonerSpellList.updateOne({ championId: payload.championId, role: roleName, gameMode: q.gameMode, spells: role.summonerspells }, { ...q, spells: role.summonerspells }, { upsert: true });

      for (const perk of role.perks)
        if (perk.selectedPerkIds.length >= 6) await PerkPage.updateOne({ championId: payload.championId, role: roleName, gameMode: q.gameMode, selectedPerkIds: perk.selectedPerkIds }, { ...q, ...perk }, { upsert: true }).exec();
        else console.log("A perk page hasn't been uploaded because of missing perks");

      for (const itemset of role.itemsets)
        if (itemset.blocks && itemset.blocks.length && itemset.blocks.length > 0) await ItemSet.updateOne({ championId: payload.championId, role: roleName, gameMode: q.gameMode, ...itemset }, { ...q, ...itemset }, { upsert: true }).exec();
    }
