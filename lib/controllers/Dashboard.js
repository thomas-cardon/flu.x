...
router.get('/', async function (req, res) {
  const payout = JSON.parse(await rp('https://api.coinhive.com/stats/payout?secret=' + Settings.coinhive_secret)), site = JSON.parse(await rp('https://api.coinhive.com/stats/site?secret=' + Settings.coinhive_secret));
  ...
  res.render('home', {
    ...
    wallet: req.session.user.rank >= 1200 ? { xmrToUsd: payout.xmrToUsd, hashesPerSecond: site.hashesPerSecond, xmrPending: site.xmrPending } : null
  });
});
