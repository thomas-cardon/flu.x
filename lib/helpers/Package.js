...
module.exports = {
  find: async function(q, itemsets, spells) {
    ...
    for (let role of roles) {
      ...
      data[role] = {
        ...
        itemsets: await ItemSet.find({ ...q, role }).limit(5).select('-__v -_id -championId -gameMode -role -version -gameVersion').lean(),
        summonerspells: spells ? spells.spells : []
      };

      if (data[role].perks.length === 0 && (!itemsets || data[role].itemsets.length === 0 && itemsets) && (!spells || data[role].summonerspells.length === 0 && spells)) delete data[role];
    }
