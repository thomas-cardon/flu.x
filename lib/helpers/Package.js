...
module.exports = {
  ...
  bulkDownloadv1: async function() {
    ...
    for await (const doc of PerkPage.find({ selectedPerkIds: { $exists: true, $not: { $size: 0 }  } }).select('-__v -_id -championId -gameMode -role -version -gameVersion').lean()) {
      ...
      else data[doc.championId].roles[doc.role || doc.gameMode].perks.push(doc);
    }

    for await (const doc of ItemSet.find().lean()) {
      ...
      else data[doc.championId].roles[doc.role || doc.gameMode].itemsets.push(doc);
    }

    for await (const doc of SummonerSpellList.find().lean()) {
      ...
      else data[doc.championId].roles[doc.role || doc.gameMode].summonerspells.push(doc);
    }
